...
import { Maybe, curry } from './containers.ts';
...
const combineName = curry((first: string, last: string): string => `${first} ${last}`);

const safeFullName = (user: User): Maybe<string> => {
  // TODO:
...
const buildDate = curry(
  (year: number, month: number, day: number): string =>
    `${year}-${pad(month)}-${pad(day)}`
);

const safeCreateDate = (
  maybeYear: Maybe<number>,
  maybeMonth: Maybe<number>,
  maybeDay: Maybe<number>
): Maybe<string> => {
...
const multiply3 = curry((a: number, b: number, c: number): number => a * b * c);
