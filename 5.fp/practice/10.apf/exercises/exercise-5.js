...
import { Maybe, Right, Left, IO, curry, liftA2, liftA3 } from './containers.js';
...
const buildProfile = curry((user, avatar, stats) => ({
  name: user.name,
...
const buildProfileApplicative = (userId) => {
  // TODO:
...
const buildProfileSequential = (userId) => {
  // TODO:
  // return new IO(() => {
  //   const maybeUser = fetchUser(userId).unsafePerformIO();
  //   const user = maybeUser.getOrElse(null);
...
const scenarioB = (arr, maybeIndex) => {
  // ИНСТРУМЕНТ: ??? (ap или chain?)
...
const scenarioC = (maybeA, maybeB, maybeC) => {
  // ИНСТРУМЕНТ: ??? (ap или chain?)
