import { Operators } from "./ast/constants.ts";
import {
...
export const getArithmeticHandler = (operator: ArithmeticOperator): ArithmeticHandler => {
  const handlers = {
...
): boolean => Operators[toCompareOperator].prec > Operators[existingOperator].prec;
...
export const isOperator = (s: string): s is ArithmeticOperator => s in Operators;

export const assertToken = (predicate: boolean) => {
  if (predicate) return;
